...
given an array of values and an amount of beggars, you are supposed to return
an array with the sum of what each beggar brings home,
...
def beggars(values, number_of_beggars):
    offers = values.copy()
    beggars_list = [0 for a in range(number_of_beggars)]
    if number_of_beggars == 0:
        ...
        while offers:
            for beggars in range(number_of_beggars):
                try:
                    beggars_list[beggars] += offers.pop(0)
                except IndexError:
                    break
        return beggars_list
